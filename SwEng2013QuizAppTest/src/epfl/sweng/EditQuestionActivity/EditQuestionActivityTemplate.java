...
public class EditQuestionActivityTemplate extends
		ActivityInstrumentationTestCase2<EditQuestionActivity> implements
		ITTCordinatorHelper {
	...
	protected void setUp() throws Exception {
		...
		QuizApp.getPreferences().edit()
				.putBoolean(PreferenceKeys.ONLINE_MODE, true);
	}
	...
	private void cleanUpData() {
		...
		File postDB = QuizApp.getContexStatic().getDatabasePath(
				CacheManager.POST_SYNC_DB_NAME);
		File cacheDB = QuizApp.getContexStatic().getDatabasePath(
				CacheManager.QUESTION_CACHE_DB_NAME);
		postDB.delete();
		cacheDB.delete();
	...
	public void clickAndWaitForButton(final TestCoordinator.TTChecks expected,
			final String button) {
		mCoordinator.clickAndWaitForButton(expected, button);
	}

	public void enterTextAndWaitFor(final TestCoordinator.TTChecks expected,
			final EditText et, final String text) {
		mCoordinator.enterTextAndWaitFor(expected, et, text);
	}

	public void clickAndWaitForAnswer(final TestCoordinator.TTChecks expected,
			final String answer) {
	...
	@Override
	public void clickAndGetToastAndWaitFor(TTChecks expected,
			final String button, String text) {
	...
	protected void fillCorrectQuizQuestion() {
		...
		clickAndWaitForButton(TTChecks.QUESTION_EDITED, getActivity()
				.getResources().getString(R.string.heavy_ballot_x));
