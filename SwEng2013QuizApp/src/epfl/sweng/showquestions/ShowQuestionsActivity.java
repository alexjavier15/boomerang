...
public class ShowQuestionsActivity extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		answerListener = new OnItemClickListener() {
			...
			@Override
			public void onItemClick(AdapterView<?> listAdapter,
					View view, int selectedAnswer, long arg3) {
				...
				String newText = textListener.getText()
						.toString() + question;
	...
	/**
	 * Obtains a random question thru an AsyncTask but blocks the thread
	...
	public HttpResponse fetchFirstQuestion() {
		...
		try {
			response = new HttpCommsBackgroundTask(this, false)
					.execute().get();
	...
	public void processHttpReponse(HttpResponse httpResponse) {
		...
		if (httpResponse != null
			...
			try {
				quizQuestion = JSONParser.parseJsonToQuiz(
						httpResponse,
