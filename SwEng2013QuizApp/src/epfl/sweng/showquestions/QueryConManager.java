...
public class QueryConManager extends ConnectionManager {
    ...
    public HttpResponse requete() {
        ...
        if (qCount == questionIndex) {
            try {
                JSONObject joll = JSONParser.parseKeyValuePairtoJSON("query", query);
                if (hasNext) {
                    joll.put("from", next);
                }
                postQuery(joll);
        ...
        if (questionIndex < qCount) {
            response = HttpCommsProxy.getInstance().getQueryQuestion(questionIndex);
            questionIndex++;
        } else {
        ...
        try {
            ...
            if (questionArray.length() > 0) {
                for (int i = 0; i < questionArray.length(); i++) {
                    HttpCommsProxy.getInstance().saveQuery(questionArray.getJSONObject(i).toString(), qCount);
                    qCount++;
                }
            }
