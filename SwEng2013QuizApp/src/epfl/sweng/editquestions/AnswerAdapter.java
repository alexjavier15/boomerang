		...
		if (view == null) {
			...
			holder.getAnswerText().addTextChangedListener(new TextWatcher() {
				...
				public void afterTextChanged(Editable s) {
					/*
					 * We need to check if the quiz question is valid and if so we must enable the
					 * submit button. The
					 * problem here is currently there is no way to access the submit button from
					 * here.
					 */
				}
				...
				public void onClick(View v) {
					if (isChecked >= 0) {
						getItem(isChecked).setChecked(
							activity.getResources().getString(R.string.heavy_ballot_x));
					}
					isChecked = position;
					getItem(position).setChecked(
						activity.getResources().getString(R.string.heavy_check_mark));
