...
public class AuthenticationActivity extends Activity implements
	...
	private HttpResponse confirm(HttpResponse response)
		...
		response = HttpComms.getInstance(this).postQuestion(
	...
	public void logIn(View view) {
		if (HttpComms.getInstance(this).isConnected()) {
			...
			Toast.makeText(this, "Not internet connection", Toast.LENGTH_SHORT)
					.show();
		}
	}
	...
	private HttpResponse postTequilaToken(HttpResponse tokenResponse)
		throws JSONException, IOException, NetworkErrorException {
		...
		return HttpComms.getInstance(this).postEntity(HttpComms.URL_TEQUILA,
				urlEntity);
	...
	public void processHttpReponse(HttpResponse response) {
		if (response != null) {
			...
			try {
				String sessionID = JSONParser.parseJsonGetKey(response,
						"session");
				CredentialManager.getInstance(this).writePreference(
						PreferenceKeys.SESSION_ID, sessionID);
				Toast.makeText(this, mStatusMsg, Toast.LENGTH_SHORT).show();
				Debug.out(sessionID);
	...
	private HttpResponse requestAuthToken(HttpResponse starter)
		...
		HttpResponse response = HttpComms.getInstance(this).getHttpResponse(
				HttpComms.URL_SWENG_SWERVER_LOGIN);
		if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
			state = TOKEN;
			return response;
	...
	private HttpResponse stateMachine(HttpResponse response)
		throws ClientProtocolException, IOException, JSONException,
			NetworkErrorException, AuthenticationException {
		/*
		 * if (failedCount > MAX_NUMBER_OF_FAILS) { // too many fails! reset
