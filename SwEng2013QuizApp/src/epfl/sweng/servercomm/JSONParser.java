...
public class JSONParser {
    ...
    public static QuizQuestion parseJsonToQuiz(HttpResponse response) throws HttpResponseException, JSONException,
        ...
        String[] answers = jsonArrayToStringArray(parser.getJSONArray("answers"));

        int solutionIndex = parser.getInt("solutionIndex");
        String[] tags = jsonArrayToStringArray(parser.getJSONArray("tags"));
        List<String> set = Arrays.asList(tags);

        return new QuizQuestion(id, question, Arrays.asList(answers), solutionIndex, set);
